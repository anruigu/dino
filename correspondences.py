...
def find_correspondences_images(
    image1: Image,
    image2: Image,
    num_pairs: int = 10,
    load_size: int = 512,
    ...
    # normalize
    top_k_descriptors_bb_descs1 = (
        top_k_descriptors_bb_descs1
        / np.linalg.norm(top_k_descriptors_bb_descs1, axis=1)[:, None]
    )
...
def draw_correspondences_onefig(
    points1: List[Tuple[float, float]],
    points2: List[Tuple[float, float]],
    image1: Image.Image,
    image2: Image.Image,
...
def process_image_pair(
    ...
    with torch.no_grad():
        ...
        fig1.savefig(
            curr_save_dir / "{}_corresp.png".format(name1),
            bbox_inches="tight",
            pad_inches=0,
        )
        fig2.savefig(
            curr_save_dir / "{}_corresp.png".format(name2),
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close("all")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Facilitate ViT Descriptor point correspondences."
    )
    parser.add_argument(
        "--root_dir", type=str, required=True, help="The root dir of image pairs."
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        required=True,
        help="The root save dir for image pairs results.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--layer", default=9, type=int, help="layer to create descriptors from."
    )
    parser.add_argument(
        "--bin",
        default="True",
    ...
    parser.add_argument(
        ...
        default=0.05,
        type=float,
        help="saliency maps threshold to distinguish fg / bg.",
    )
    parser.add_argument(
        "--num_pairs", default=10, type=int, help="Final number of correspondences."
    )
    ...
    with torch.no_grad():
        ...
        for pair_dir in tqdm(pair_dirs):
            curr_images = [
                x
                for x in pair_dir.iterdir()
                if x.suffix.lower() in [".jpg", ".png", ".jpeg"]
            ]
            assert (
                len(curr_images) == 2
            ), f"{pair_dir} contains {len(curr_images)} images instead of 2."
            ...
            fig1, fig2 = draw_correspondences(points1, points2, image1_pil, image2_pil)
            fig1.savefig(
                curr_save_dir / f"{Path(curr_images[0]).stem}_corresp.png",
                bbox_inches="tight",
                pad_inches=0,
            )
            fig2.savefig(
                curr_save_dir / f"{Path(curr_images[1]).stem}_corresp.png",
                bbox_inches="tight",
                pad_inches=0,
            )
            plt.close("all")
